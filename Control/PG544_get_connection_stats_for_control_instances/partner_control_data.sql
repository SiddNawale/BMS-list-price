...
select
    ...
    ac.CurrencyCode,
    ISNULL(
        ISNULL(LP.Price, 0.00) / NULLIF(ic.billingintervalmonthcount, 0),
        0.00
    ) MRR
from
    Reporting.InstancesWithActiveLicense iwal
    left join Account ac on ac.AccountID = iwal.AccountID
    left join InstanceChange ic on ic.InstanceID = iwal.InstanceID
    LEFT JOIN LicensePrice LP ON iwal.LicenseTypeID = LP.LicenseTypeID
