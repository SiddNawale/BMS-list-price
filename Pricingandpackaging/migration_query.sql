...
WITH customer_roster AS (
        SELECT
                ...
                ) as COMMAND_PREFFRED_SERVER_MRR,
                -----------------------------------------------------------------
                -- Other units
                SUM(
                        IFF(
...
FROM
        ...
        LEFT JOIN DATAIKU.DEV_DATAIKU_STAGING.PNP_DASHBOARD_AUTOMATE_AND_MANAGE_CALC_C amc on amc.COMPANY_ID = cr.COMPANY_ID --merged queries
